...
#temel_paket_dosyasi = "temel_paketler"
temel_paket_dosyasi = "pisi_base"

with open(temel_paket_dosyasi) as f:
    TEMEL_SISTEM = f.readlines()
...
class Index:
    ...
    def checkHash(self):
        ...
        if os.path.exists("%s.index.sha1sum" % self.name):
            yeniHash = urllib.urlopen("%s.sha1sum" % self.url).readlines()[0]
...
class Chroot:
    ...
    def dbus(self, start=False):
        if start == True:
            ...
            self.runCommand(
                "/sbin/start-stop-daemon -b --start  --pidfile /run/dbus/pid --exec /usr/bin/dbus-daemon -- --system")
    ...
    def installpackages(self, pkglist1 = None):
        if pkglist1 != None:
            liste = pkglist1
        else:
            liste = self.liste
...
if (__name__ == "__main__"):
    ...
    for repo in repolar:
        x.addRepo(repolar[0][0], repolar[0][1])
